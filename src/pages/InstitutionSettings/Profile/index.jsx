...
const Profile = () => {
	...
	const { model, token } = useSelector((s) => s.auth);
	...
	const SaveAlertDialog = () => {
		...
		const handleSave = () => {
			if (
				currentValues.locationLat === null &&
				currentValues.locationLong === null
			) {
				toast({
	...
	const onSuccess = (position) => {
		if (
			position.coords.latitude !== currentValues.locationLat &&
			position.coords.longitude !== currentValues.locationLong
		) {
			setCurrentValues({
				...currentValues,
	...
	const handleSubmit = () => {
		...
		axios
			.put(`/api/0.1/institution/${model.id}`, formData, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			})
						...
						<div>
							<div className={styles.halfField}>
								<div className={styles.leftLabel}>
									...
									<p className="caption">
										Upload your display picture to share with users
									</p>
								</div>
									...
									<div className={styles.imageContainerRight}>
										...
										{currentValues.avatarPhoto === "" ? (
											<Button
												...
												disabled={isImageRemoveDisabled}
											>
												Remove picture
											</Button>
										) : (
							...
							<div className={styles.halfField}>
								<div className={styles.leftLabel}>
									...
									<p className="caption">
										Edit your description to let users get to know you better
									</p>
								</div>
								<Textarea
									name="description"
							...
							<HR />
							<div
								style={{ marginTop: "60px", marginBottom: "28px" }}
								className={styles.halfField}
							>
								<div className={styles.leftLabel}>
									<h3 className="heading-3">Account Information</h3>
									<p className="caption">
										Update your Information to display on your profile
									</p>
								</div>
								<div>
									...
									<div className={styles.twoFields}>
										...
										<Tooltip
											hasArrow
											label="Sorry. This feature is not yet available."
											bg={`var(--color-very-light-grey)`}
											color={`--color-black`}
											borderRadius="4px"
											fontFamily="Raleway"
										>
									...
									<div className={styles.twoFields}>
										...
										{currentValues.locationLat !== values.locationLat &&
										currentValues.locationLong !== values.locationLat ? (
											<Button
												size="small"
												color="brand-default"
							...
							>
								<div className={styles.leftLabel}>
									...
									<p className="caption">
										Add your website and social media pages to connect with
										users
									</p>
								</div>
								<div>
