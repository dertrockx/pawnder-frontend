...
const ManageStoryList = () => {
	...
	const { model = {}, loginType, token } = useSelector((s) => s.auth);
	...
	const handleDelete = (id) => {
		axios
			.delete(`/api/0.1/story/${id}`) // might need headers
			.then(() => {
				toast({
	...
	const handlePublish = (id) => {
		axios
			.put(`/api/0.1/story/${id}?publish=1`) // might need headers
			.then(() => {
				toast({
			...
			{loginType === "user" || loginType === null ? (
				...
				<Router>
					<Switch>
													...
													<div className={styles.options}>
														<div>
															<Menu>
																...
																<MenuList>
																	<MenuOptionGroup
																		onChange={(value) => setFilterStory(value)}
																		value={filterStory}
																		type="radio"
																	>
																		<MenuItemOption
														...
														<div className={styles.searchBar}>
															<InputGroup>
																<InputLeftElement
																	pointerEvents="none"
																	children={
																		<IoSearch color="rgb(187, 200, 212)" />
																	}
																/>
																<Input
																	name="search"
														...
														<Link to="manage-stories/create">
															<IconButton
																...
																style={{
																	...
																	bottom: "40px",
																	right: "60px",
																}} // For positioning floating action button to bottom right of screen
															/>
														</Link>
													</Tooltip>
												</div>
						...
						<Route
							exact
							path="/institution/manage-stories/create"
							component={StoryDetails}
						/>{" "}
						{/* This should always be above the route for with :id. */}
						<Route
							exact
							path="/institution/manage-stories/:id"
							render={() => <StoryDetails data={storiesCopy} />}
						/>{" "}
						{/* Might not pass data anymore since magre-refetch naman doon sa details page */}
					</Switch>
				</Router>
			)}
		</>
