...
function ManageStoryDetails({ data }) {
	...
	const putRequestPublish = () => {
		...
		axios
			.put("http://localhost:8081/api/0.1/story/" + id + "?publish=1", fd)
			.then(() => {
				setSuccess(true);
			})
			.catch(() => {
	...
	const handlePublish = () => {
		if (
			...
			if (id === undefined) {
				...
				axios
					.post("http://localhost:8081/api/0.1/story/", fd)
					.then((response) => {
						const storyId = response.data.story.id;
						axios
							.put(
								"http://localhost:8081/api/0.1/story/" + storyId + "?publish=1"
							)
							.then(() => {
								setLoading(false);
								toast({
									title:
										"Successfully published story. Redirecting to show list page please wait.",
									status: "success",
									position: "top",
									duration: 5000,
									isClosable: true,
	...
	const handleDraft = (e) => {
		...
		if (
			...
			if (id === undefined) {
				...
				axios
					...
					.then(() => {
						...
						toast({
							title:
								"Successfully saved story as draft. Redirecting to show list page please wait.",
							status: "success",
							position: "top",
							duration: 5000,
							isClosable: true,
			...
			//put request if story is not newly created
				...
				fd.append("headlinePhoto", storyInfo.headlinePhoto);
				axios
					.put("http://localhost:8081/api/0.1/story/" + id, fd)
					.then(() => {
						setLoading(false);
						toast({
							title:
							...
							<div className={styles.buttons}>
								...
								<Modal onClose={onCancelClose} isOpen={isCancelOpen} isCentered>
									...
									<ModalContent>
										...
										<ModalFooter>
											<Button onClick={onCancelClose} mr={3}>
												Continue Editing
											</Button>
											<Button colorScheme="red" onClick={movePages}>
												Cancel Story
											</Button>
										</ModalFooter>
									</ModalContent>
								...
								<Modal onClose={onDraftClose} isOpen={isDraftOpen} isCentered>
									...
									<ModalContent>
										...
										<ModalBody>
											<p className="paragraph">
												Are you sure you want to save your story as draft?
											</p>
										</ModalBody>
										<ModalFooter>
											<Button colorScheme="red" mr={3} onClick={onDraftClose}>
												Cancel
											</Button>
											<Button
												colorScheme="yellow"
												color="white"
												onClick={sendingDraft}
											>
												Save as Draft
											</Button>
										</ModalFooter>
								...
								>
									...
									<ModalContent>
										...
										<ModalBody>
											...
											<br />
											<p className="paragraph">
												This will allow users to read your story. Besides the
												tags, you can still edit your post after this. Are you
												sure?
											</p>
										</ModalBody>
										<ModalFooter>
											<Button colorScheme="red" mr={3} onClick={onPublishClose}>
												Cancel
											</Button>
											<Button
												colorScheme="green"
												type="submit"
												onClick={publishingNewStory}
											>
												Publish
											</Button>
										</ModalFooter>
