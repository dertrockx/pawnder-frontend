...
function UserSignupPage() {
	...
	function handleFormSubmit(e) {
		...
		if(!/\S+@\S+\.\S+/.test(email) || password !== confirmPassword || password === "" || confirmPassword === "" || email === ""){
		...
		<div className={styles.page}>
			...
			>
				...
				>
					...
					/><br/>
					<label className={`${styles.column} ${styles.colleft}`}>Password </label>{passwordError && <div className={`${styles.error} ${styles.column} ${styles.colright}`}>Password does not match!</div>}
					<BasicInput 
						type="password"
						name="password"
					...
					<BasicInput 
						...
						required="true"
					/><br/><br/>
					<Button onClick={handleFormSubmit} color="brand-default" size="small" block>Signup</Button><br/>
					<div 
						className={`
							caption
							${styles.header}
						`}
					>
						Already have an account? <BasicLink to="/user/login" label="Log in"/>
					</div>
				</form>
			</div>
		</div>
